...
- [x] `backend/internal/ai/` ディレクトリ作成
- [x] `Client` インターフェース定義 (`Generate(ctx, prompt) (text, latency, error)`)
- [x] Gemini 実装構造体 + コンストラクタ (`NewGeminiClient(apiKey string, httpClient *http.Client)`)
- [x] タイムアウト付き `http.Client` 作成
- [x] リトライロジック（`context.DeadlineExceeded` / 5xx）
- [x] エラーハンドリング（分類: auth/network/api）
- [x] 単体テスト (httptest.Server)
- [x] README or inline comment に利用例追記
